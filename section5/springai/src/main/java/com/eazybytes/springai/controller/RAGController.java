...
//        //set up the configuration of searchRequest
//        SearchRequest searchRequest =
//                SearchRequest.builder().query(message).topK(3).similarityThreshold(0.5).build();
//        //search in the vectorStore and generate the similarDocs
//        List<Document> similarDocs = vectorStore.similaritySearch(searchRequest);
//        //fetch the actual text information present in each of the document object collected as a
//        //single string, each document text is going to be separated by using a line separator
//        //this similarContext will be passed as an input into the PromptTemplate
...
//                .collect(Collectors.joining(System.lineSeparator()));
        String answer = chatClient.prompt()
//                .system(promptSystemSpec -> promptSystemSpec.text(hrSystemPromptTemplate)
//                        .param("documents", similarContext))
                .advisors(a -> a.param(CONVERSATION_ID, username))
                .user(message)
                .call().content();
